...
因此，我们的"自研脚本"或工具链必须实现几何简化（Mesh Decimation）和构件的层级化组织，以生成高质量的LOD。例如，当视点远离建筑时，只加载一个低多边形的建筑外壳瓦片；随着视点靠近，逐步加载包含外墙细节、窗户、乃至室内构件的更高精度瓦片。若忽略LOD的生成，即便格式转为3D Tiles，其性能表现也可能不尽人意，违背了采用此技术的初衷。
